/*
 ...
 Source Server         : dev.aimstek.cn
 Source Server Type    : MySQL
 Source Server Version : 80021 (8.0.21)
 Source Host           : dev.aimstek.cn:32150
...
CREATE TABLE `logic_instance` (
  ...
  `parentLogicId` varchar(36) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '父逻辑id',
  `parentBizId` varchar(36) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '父业务标识',
  ...
  `isRunning` tinyint(1) DEFAULT '0' COMMENT '是否运行中',
  `createTime` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '实例创建时间',
  `serverTime` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '服务器时间',
  `startTime` datetime(3) DEFAULT CURRENT_TIMESTAMP(3) COMMENT '开始时间',
  `stopTime` datetime(3) DEFAULT NULL COMMENT '结束时间',
  ...
  KEY `idx_bizId_parBizId_serverTime` (`bizId` DESC,`parentBizId` DESC,`serverTime` DESC)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
